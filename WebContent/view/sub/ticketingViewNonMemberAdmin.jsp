		...
		<tbody>
			...
			<% if (memberTicketings != null && memberTicketings.size() > 0) { %>
				<% for (int i = 0; i < memberTicketings.size(); i++) { %>
					...
					<tr id="<%=vo.getWatCd() %>">
						<td><%=vo.getnMemCd() %></td>
						<td><%="0" + vo.getNonMemberVO().getnMemNum() %></td>
			...
			<% } else { %>
				<tr>
					<td colspan="5" class="text-center">비회원 예매내역이 없습니다.</td>
				</tr>
			<% } %>
		</tbody>
	</table>
...
<script>
	...
	$('.btn_cancle').on('click', function(){
		...
		$.ajax({
			type: 'post',
			url: '<%=request.getContextPath() %>/nonMemberTicketingAdminCancle.do',
			...
			success: function(data){
				if (data.isCancled == 'Y') {
					...
					if ($('#ticketingViewTbl tbody').find('tr').length == 0) {
						...
						html += '<tr>';
						html += '	<td colspan="5" class="text-center">회원 예매내역이 없습니다.</td>';
						html += '</tr>';
