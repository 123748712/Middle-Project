...
<div class="nonMember_wrap">
  <div class="container">
    ...
    <div class="topbox_st1">
    	...
    	<div class="inner">
    		<h3>
		      <strong class="color_red">STEP 1</strong> 개인정보(휴대폰번호, 법정생년월일, 비밀번호) 입력
		    </h3>
  ...
  <form class="form-horizontal" method="post" action="nonMember.do">
    <div class="form-group">
      ...
      <div class="col-sm-10">
        <input type="text" class="form-control" id="birth" maxlength="8" required="required" name="nMemBir">
      </div>
    </div>
    ...
    <div class="form-group">
      <label class="control-label col-sm-2" for="certifi">인증번호(4자리)</label>
      <div class="col-sm-6">
    ...
    <div class="form-group">
      <label class="control-label col-sm-2" for="pwChk">비밀번호확인</label>
      <div class="col-sm-8">
      ...
      <div class="text-center">
    	<button type="button" class="btn btn-default" id="nonRes">비회원 예매하기</button>
      </div>
  </form>
</div>
</div>
  ...
  let rest=""
    $("#txtMobileBtn").on("click", function() {
		...
		rest = setInterval(function() {
			...
			$(".time").text(min + ":" + sec).css("color", "red");
			time--;
    ...
    $("#nonRes").on("click", function() {
    	if ($("#certifi").val() == "" || $("#txtMobileChkRst").text() == "" || $("#txtMobileChkRst").text() == "인증번호 불일치") {
    		alert("인증번호를 확인해주세요.");
    		return false;
    		
		} else if ($("#pwChkPrint").text() == "비밀번호 불일치" || $("#pwChkPrint").text() == "") {
			alert("비밀번호를 확인해주세요.");
			return false;
		}
    	this.form.submit();
