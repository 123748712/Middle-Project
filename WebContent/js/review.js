	...
	$.ajax({
		...
		success : function(res){
			...
			$.each(res.datas, function(i,v){
				...
				code += '      <a idx="' + v.mov_rev_num + '" class="list" data-toggle="collapse" data-parent="#accordion" href="#collapse' + v.mov_rev_num + '">';
				code += 		v.mov_rev_tit + '</a>';      
				code += '    </h4>';
				code += '  </div>';
				code += '  <div id="collapse' + v.mov_rev_num + '" class="panel-collapse collapse">';
				code += '    <div class="panel-body pbody">';
				code += '		<div class="p1">작성자 : ' + v.mov_rev_wri + '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
				code += '		</div>';
				code += '		<div class="p2">';
				code += '  			<input idx="' + v.mov_rev_num + '" type="button" name="modify" value="수정" class="action btn btn-warning btn-sm">';
				code += '			<input idx="' + v.mov_rev_num + '" type="button" name="delete" value="삭제" class="action btn btn-danger btn-sm">';
				code += '		</div>';
				code += '		<div class="p3">' + v.mov_rev_con + '</div>';
				code += '    </div>';
				code += '  </div>';
				code += '</div>';
			});
...
$(function(){
	...
	$('#reviewList').on('click','.action', function(){
		
		let vname = $(this).attr('name');
		vidx = $(this).attr('idx');
		//게시글 삭제
		if(vname == 'delete'){
			// 회원만 삭제 가능하게 처리. 2022.03.23. OSH 추가
			if ($('[name="loginId"]').val().trim() == '') {
				alert('삭제 권한이 없습니다.');
				return false;
			}
		...
		//게시글 수정
			...
			// 회원만 수정 가능하게 처리. 2022.03.23. OSH 추가
			if ($('[name="loginId"]').val().trim() == '') {
				alert('수정 권한이 없습니다.');
				return false;
			}
...
writeServer = function(){
	$.ajax({
		url : '/LotteCgvBox/ReviewWrite',
		method : 'post',
		data : $('form').serialize(),
		success : function(res){
			if (res.foundXXwords > 0) {
				alert('욕설을 등록할 수 없습니다.');
				return false;
			}
...
updateServer = function(){
	$.ajax({
		url : '/LotteCgvBox/ReviewUpdate',
		type : 'post',
		data : modf,
		success : function(data){
			if (data.foundXXwords > 0) {
				alert(data.msg);
				location.reload();
			}
		},
