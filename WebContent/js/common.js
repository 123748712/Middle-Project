$(function(){
	...
	$('#openMenu').on('click', toggleMobMenu);
	
	function toggleMobMenu(){
		$('#mobMenu').stop().fadeToggle(200);
	}
});
...
$(window).on('load resize', function(){
	...
	if ($(this).outerWidth() <= 1400) {
		if ($('#mobMenu').find('nav').length == 0) {
			$('#header .loginMsg').clone().appendTo('#mobMenu');
			$('#header .util').clone().appendTo('#mobMenu');
			$('#menu nav').clone().appendTo('#mobMenu');
			$('#mobMenu').find('[class^="depth"]').removeAttr('style');
		} 
	} else {
		$('#mobMenu').hide();
	}
